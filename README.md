...
Both badges are adrkit's own recipe ([ADR-0025](https://github.com/mbeacom/adrkit/blob/d9ce9e18fbe95525b4e3b2780bfe33352a5ab7f2/docs/adr/0025-ship-badges-as-recipes-over-existing-output.md),
...
- **Repin to `e3155eaa` / adrkit `v0.7.0` (2026-08-12)** — Action
...
- **Repin to `3e40675e` / adrkit `v0.13.0` (2026-08-30)** — the current pin.
  This release replaces the remaining `affects` locale-sensitive sorts with
  code-unit ordering, adds stale-marker warning logic, and makes the
  governing-decisions Action metadata Marketplace-compatible. All six
  local validation surfaces pass, and the queue Action plus committed badge
  reports remain byte-identical. No live workflow was dispatched and no
  pull-request checks are claimed at this pin. See
  [Re-validation against `3e40675e`](#re-validation-against-3e40675e-adrkit-v0130-2026-08-30).
- **Repin to `2f19524f` / adrkit `v0.12.0` (2026-08-27)** — the preceding pin.
  This release changes the queue kernel, marker declaration accounting, and the
  governing-decisions Action's logs and comment rendering. All six local
  validation surfaces pass, and the committed badge reports remain
  byte-identical. No live workflow was dispatched and no pull-request checks
  are claimed at this pin. See
  [Re-validation against `2f19524f`](#re-validation-against-2f19524f-adrkit-v0120-2026-08-27).
- **Repin to `9a4bdf89` / adrkit `v0.11.0` (2026-08-26)** — the preceding pin.
...
- **Repin to `0bd7200b` / adrkit `v0.10.0` (2026-08-25)** — the preceding pin.
  ...
  workflow was dispatched and no pull-request checks are claimed at this pin.
  See
  [Re-validation against `0bd7200b`](#re-validation-against-0bd7200b-adrkit-v0100-2026-08-25).
- **Repin to `e66b43dd` / adrkit `v0.9.0` (2026-08-24)** — the preceding pin.
  Queue-adjacent source moved: the `--as-of` resolver is now exported from
  `@adrkit/core`, and path-separator normalization in `adr check` is
  platform-aware. The managed queue Action itself is byte-identical across the
  repin (entrypoint, metadata, and bundle). All six local validation surfaces
  pass, and a live managed-issue dispatch refreshed and self-verified issue
  [#3](https://github.com/mbeacom/adrkit-t018-dogfood/issues/3) at this pin. The
  fail-closed workflow was not re-dispatched. See
  [Re-validation against `e66b43dd`](#re-validation-against-e66b43dd-adrkit-v090-2026-08-24).
- **Repin to `d9ce9e18` / adrkit `v0.8.0` (2026-08-15)** — the narrowest repin
  this repository has recorded. **No `packages/ci/` file
  changed at all** across `e3155eaa…d9ce9e18` (0 of 73 changed files), and the
  only `src/` movement in any publishable package is two version-string
  constants. All four validation scripts were nonetheless re-run locally and
  pass, because "the diff looks safe" is an inference and a green script is an
  observation. The run also found that the unscoped `adrkit` npm package named in
  the v0.8.0 release notes **is not published**. See
  [Re-validation against `d9ce9e18`](#re-validation-against-d9ce9e18-adrkit-v080-2026-08-15).
...
```
3e40675ed6f513d9712b1dccaa68034d649d1eb9
```

This is the commit behind adrkit's **`v0.13.0`** release tag, verified by
dereferencing the annotated tag rather than trusting the release page:

```console
$ gh api repos/mbeacom/adrkit/git/ref/tags/v0.13.0 --jq '.object.type, .object.sha'
tag
39880b524a72f2b19bd01e3c41d3212af3c3d47f
$ gh api repos/mbeacom/adrkit/git/tags/39880b524a72f2b19bd01e3c41d3212af3c3d47f --jq '.object.sha'
3e40675ed6f513d9712b1dccaa68034d649d1eb9
```

It supersedes `2f19524f07938f1a7841b9f58bbcd1313e60a4dc` (`v0.12.0`), which
superseded `9a4bdf89680b1a5b358fba0f2c8c88b9f5ac7877` (`v0.11.0`), which
superseded `0bd7200bbbe98969f6ce978e5e8f44af45b1f866` (`v0.10.0`), which
...
| `.github/workflows/arb-queue.yml` | Phase 6 dedicated Action workflow: creates/updates the managed ARB queue issue via `mbeacom/adrkit/packages/ci/queue@3e40675ed6f513d9712b1dccaa68034d649d1eb9`, then self-verifies the result via `scripts/verify-managed-queue-issue.sh`. |
...
| `scripts/validate-spec-kit-extension.sh` | Self-verifying, fail-closed assertions for the Spec Kit extension: install and rendering, the hook contract, packaging hygiene, behavior against this repo's real corpus, zero mutation, and four consumer-facing failure modes that must produce no side effect. 49 assertions per upstream version. |
...
1. Clones `mbeacom/adrkit` and checks out the pinned commit
   `3e40675ed6f513d9712b1dccaa68034d649d1eb9` into a temporary directory
...
4. Records the binary's self-reported version (`adr --version`, currently
   `0.13.0`) alongside the pinned SHA, and fails if it exits non-zero or
   ...
   `--version` entirely should. That design has now paid off repeatedly — the
   version moved `0.2.0` → `0.4.0` → `0.6.0` → `0.8.0` → `0.9.0` →
   `0.10.0` → `0.11.0` → `0.12.0` → `0.13.0`, and the check passed unchanged each time,
   recording the new value without any edit to the assertion.
5. Runs:
   ```bash
...
1. **The tarball is content-pinned.** `scripts/validate-mcp.sh` records the
   registry's `sha512` integrity for `@adrkit/mcp@0.13.0` and verifies the
...
2. **The tarball is tied to the pinned commit.** `@adrkit/mcp` ships its own
   ...
   where `package/` is the extracted `@adrkit/mcp@0.13.0` tarball and `fromgit/`
   is `git archive 3e40675ed6f513d9712b1dccaa68034d649d1eb9 packages/mcp/src`.
...
"@adrkit/mcp@0.8.0"` — which performs *its own* registry resolution, so the bytes
...
the filename is `adrkit-mcp-0.13.0.tgz`, where the version is not adjacent to the
...
second, independent `npx -y @adrkit/mcp@0.8.0` — which re-resolves by name and
...
|---|---|
| `MCP-1`–`MCP-2` | The server identifies as `@adrkit/mcp` and reports **0.13.0**. This is a consistency check, not provenance — provenance comes from having executed the sha512-verified install. |
...
1. `id: queue` runs the packaged
   `mbeacom/adrkit/packages/ci/queue@3e40675ed6f513d9712b1dccaa68034d649d1eb9`
...
### Live dispatch at `e66b43dd` (adrkit v0.9.0)

The managed-issue workflow was dispatched against branch
`mbeacom-adrkit-0-9-dogfood` at commit `d54c571`:
[`32784931152`](https://github.com/mbeacom/adrkit-t018-dogfood/actions/runs/32784931152),
conclusion `success`. Its Action-download step resolved the exact pinned
`e66b43dd87d49648a7b28630fa9ecfff65225f2e` queue Action, and its independent
verification step reported:

- exactly one managed issue across OPEN+CLOSED issues, issue `#3`;
- the Action's `issue-number` output also identified `#3`;
- every body/metadata assertion passed;
- body SHA-256
  `336128ee191b65787e92b2227b1840c9366e67fc8a570c0033d04374f589b214`;
- `updatedAt: 2026-08-24T22:29:57Z`.

The issue now renders `# ARB Queue — 2026-08-24`, the unchanged corpus
fingerprint `1664c5af…4bd936`, three items, and zero corpus findings. This was a
real update rather than an idempotent no-op because the Action's default
`asOf` is the dispatch date and that date appears in the rendered heading. All
three items were already `overdue`, so no SLA-state transition is attributed to
the repin.

### Live dispatch under the `c3dff3a7` pin

> **Superseded as the most recent dispatch.** `arb-queue.yml` has since been
> re-dispatched at the then-current `e66b43dd` pin — run
> [`32784931152`](https://github.com/mbeacom/adrkit-t018-dogfood/actions/runs/32784931152),
...
> repository now pins `3e40675e` (adrkit `v0.13.0`). The workflow was last
...
> re-dispatched at `e3155eaa`, `d9ce9e18`, `e66b43dd`, `0bd7200b`,
> `9a4bdf89`, `2f19524f`, or `3e40675e`**. At the current repin, the queue
> Action metadata, source entrypoint, and executable bundle remain byte-identical
> to `2f19524f`; the bundle had already changed after the last live dispatch.
> The older live result is therefore retained as history, not carried forward as
> v0.13.0 evidence — see
> [What was not exercised at this pin](#what-was-not-exercised-at-this-pin-3e40675e).
...
| `adr --help` / `--version` | Added at `bbe63e01` (resolved defect 4). At that pin `adr --version` printed `0.2.0`; it printed `0.4.0` at `c3dff3a7`, `0.6.0` at `c5dc677f`, `0.8.0` at `d9ce9e18`, `0.9.0` at `e66b43dd`, `0.10.0` at `0bd7200b`, `0.11.0` at `9a4bdf89`, `0.12.0` at `2f19524f`, and prints `0.13.0` at the current `3e40675e` pin. Recorded by `scripts/validate-queue.sh` on every run, without asserting the literal string. |
...
## Re-validation against `3e40675e` (adrkit v0.13.0) (2026-08-30)

The repin from `2f19524f` to `3e40675e` spans the v0.13.0 release. The upstream
diff adds an ADR-aware code-review skill, replaces the remaining
locale-sensitive `affects` sorts with code-unit ordering, adds warning logic
for source markers that name historical decisions, and makes the
governing-decisions Action metadata Marketplace-compatible. The queue kernel,
queue Action, and CLI queue command are unchanged.

**Scope caveat, stated up front.** Six validation surfaces were run locally.
The Bun-backed surfaces ran under installed Bun `1.4.0`;
`validate-queue.sh` warned that the repository expects `1.3.14` and continued,
so CI's exact Bun pin has not yet been observed on this change. No GitHub
workflow was dispatched and no pull-request checks are claimed. The task
reported that the hosted cloud-agent MCP setting was already updated on
GitHub, but that UI state has no repository read API and was not independently
observed here.

### What changed across `2f19524f` → `3e40675e`

A local Git comparison reports 56 changed files, 2,150 insertions, and 281
deletions. The surfaces relevant here break down as:

| File | Comparison | Relevance |
|------|------------|-----------|
| `packages/ci/queue/action.yml` | byte-identical (SHA-256 `9e71b551…`) | Queue Action metadata and inputs/outputs unchanged |
| `packages/ci/dist/queue-action.js` | byte-identical (SHA-256 `416c9ac4…`) | Packaged queue Action executable unchanged |
| `packages/core/src/queue/**` | no changed files | Queue report construction and item findings unchanged |
| `packages/cli/src/queue.ts` | byte-identical | CLI queue wrapper unchanged |
| `packages/core/src/affects/{index.ts,matchers/package.ts}` | changed | Record ids, fired matchers, and dependency snapshots now use code-unit ordering instead of locale-sensitive `localeCompare` |
| `packages/core/src/markers/resolve.ts` | changed (Git blob `a2bd8a53…` → `e338242d…`) | Markers naming `superseded`, `rejected`, or `deprecated` records now emit advisory `stale-marker` warnings; a resolvable supersession chain names its terminal live successor without silently rebinding the marker |
| `packages/ci/{action.yml,src/comment.ts,dist/index.js}` | changed | Marketplace-compatible Action metadata and generalized “Marker claims needing attention” rendering ship in the governing-decisions Action; its executable bundle changed |
| `packages/cli/src/index.ts` | changed | Version moved to `0.13.0`, and `explain --help` now documents stale-marker behavior |
| `packages/mcp/src/server.ts` | version-only change (`0.12.0` → `0.13.0`) | The published MCP artifact still requires npm↔commit verification |

The `v0.13.0` tag was observed as an annotated tag resolving to
`3e40675ed6f513d9712b1dccaa68034d649d1eb9`; GitHub's tag verification API
reported `verified: true` with reason `valid`. The moving `v0` tag was observed
at the same commit. The registry `sha512` for `@adrkit/mcp@0.13.0` was observed
and independently recomputed from the packed tarball as
`sha512-aL2X38xNmriULdAlf11lMakb6FKYNsQOdzDPQwPO+6W1JO1KXgBxfVtpmEuHpZqMp9dJQH1qsi0ebd05CtlFhQ==`.
The `v0.13.0` GitHub release has no assets, so the Spec Kit matrix retains the
separately published `spec-kit-v0.1.3` asset and its existing sha256 while
moving the installed CLI to `@adrkit/cli@0.13.0`.

### What was run, and what it reported

| Validation | Observed result at `3e40675e` |
|------------|--------------------------------|
| `scripts/validate-queue.sh` | Pass under Bun `1.4.0` with the expected-version warning. `adr --version` reports `0.13.0`; 15 records lint with 0 errors and 0 warnings; all three tiers and the fixed `overdue`/`due`/`within-sla` states reproduce. |
| `scripts/validate-mcp.sh` | Pass under Node `22.22.2`. The registry `sha512` matched `@adrkit/mcp@0.13.0`, package `src/` was byte-identical to the pinned commit, the 28 config and 28 stdio JSON-RPC assertions passed, and `docs/adr` ended byte-identical. |
| `scripts/validate-markers.sh --self-test` | Pass under Bun `1.4.0`: 26 assertions across 8 fixtures; 17 perturbations falsified 22 assertions, with the remaining 4 non-falsifiable by construction. |
| `scripts/validate-badge-reports.sh` | Pass under Bun `1.4.0`: 5 assertions. Both committed reports are byte-identical to regeneration at the new pin, so no `.adrkit/*.json` edit was needed. |
...
| `scripts/validate-spec-kit-extension.sh` | Pass, 49 assertions and zero failures at both Spec Kit `0.14.4` and `0.15.1`, using the sha256-pinned `spec-kit-v0.1.3` asset and `@adrkit/cli@0.13.0`; both mutation checks passed. |

The Spec Kit runs used a temporary committed snapshot of the repin so the
`MUT-2` assertion checked a clean reference repository rather than being
weakened or skipped.

### What was *not* exercised at this pin (`3e40675e`)

- **Neither queue Action workflow was dispatched.** Its metadata and executable
  bundle are byte-identical to v0.12.0, but that is an upstream diff observation,
  not a live v0.13.0 run.
- **No pull-request or `main` CI run is claimed.** The results above are local
  observations, and the Bun-backed runs used `1.4.0` rather than CI's pinned
  `1.3.14`.
- **The hosted cloud-agent MCP setting was not read back.** Its update is
  user-reported; the checked-in canonical config and published package were
  validated here.
- **The new stale-marker branch is absent from this corpus.** The corpus has no
  superseded, rejected, or deprecated records, so the behavior was identified
  from the upstream diff rather than observed in marker output here.
- **Locale independence was not independently falsified here.** Existing ids,
  matchers, and dependency data reproduced the expected output under the local
  environment; no cross-locale synthetic corpus was run.
- **The governing-decisions Action changes were not exercised live.** No PR
  comment established the new marker-claims rendering or Marketplace metadata
  at this pin.
- **Dependency closures remain only behaviorally covered.** The top-level MCP
  tarball and Spec Kit asset are content-pinned; npm-resolved dependencies are
  not.

---

## Re-validation against `2f19524f` (adrkit v0.12.0) (2026-08-27)

The repin from `9a4bdf89` to `2f19524f` spans seven upstream commits. The
release adds trusted-gate enforcement and Action-tag recovery, emits a generated
manifest, bounds marker declarations, surfaces marker scan health in CI comments,
and corrects one queue item-finding edge case. The queue Action bundle and core
check behavior therefore changed; this is not a version-only repin.

**Scope caveat, stated up front.** Six validation surfaces were run locally.
The Bun-backed surfaces ran under installed Bun `1.4.0`; `validate-queue.sh`
warned that the repository expects `1.3.14` and continued, so CI's exact Bun pin
has not yet been observed on this change. No GitHub workflow was dispatched and no
pull-request checks are claimed. The task reported that the hosted cloud-agent
MCP setting was already updated on GitHub, but that UI state has no repository
read API and was not independently observed here.

### What changed across `9a4bdf89` → `2f19524f`

The GitHub comparison API reports 66 changed files, 5,042 additions, and 147
deletions. The surfaces relevant here break down as:
...
| `packages/ci/dist/queue-action.js` | changed (Git blob `04b63e4b…` → `e91c7040…`; 1,735,334 → 1,735,406 bytes) | Executable queue Action now includes the corrected item-finding condition |
| `packages/core/src/queue/kernel.ts` | changed (Git blob `3e8588c0…` → `7a762957…`) | A record with top-level `reviewBy` but no `review` block now receives `item.tier-absent` instead of silently carrying no tier finding |
| `packages/core/src/{markers/read.ts,markers/scan.ts,check/index.ts}` | changed | Marker declarations are bounded at 64 per file and 10,000 per batch, with exact retained/omitted accounting and a warning finding when declarations are omitted |
| `packages/ci/src/{action.ts,comment.ts}` | changed | The governing-decisions Action logs declaration counts and renders marker scan-health and unbound-claim advisories |
| `packages/cli/src/queue.ts` | byte-identical (Git blob `94e840fd…`) | The CLI queue wrapper is unchanged; its core report builder moved as described above |
| `packages/mcp/src/server.ts` | version-only change (`0.11.0` → `0.12.0`) | The published MCP artifact still requires npm↔commit verification |

The `v0.12.0` tag was observed as an annotated tag resolving to
`2f19524f07938f1a7841b9f58bbcd1313e60a4dc`; GitHub's tag verification API
reported it as `unsigned`. The moving `v0` tag was observed at the same commit.
The registry `sha512` for `@adrkit/mcp@0.12.0` was observed and re-verified as
`sha512-+7cyLPcBUMiDzSRIfNvBogkNOzRh9SJLaGYbvJFr14PB/qEiFXvSN2Uucuv4Pr/ZJXJhQPUMHnU/MzFWK0Gbag==`.
The `v0.12.0` GitHub release has no assets, so the Spec Kit matrix retains the
separately published `spec-kit-v0.1.3` asset and its existing sha256 while moving
the installed CLI to `@adrkit/cli@0.12.0`.

### What was run, and what it reported

| Validation | Observed result at `2f19524f` |
|------------|--------------------------------|
| `scripts/validate-queue.sh` | Pass under Bun `1.4.0` with the expected-version warning. `adr --version` reports `0.12.0`; 15 records lint with 0 errors and 0 warnings; all three tiers and the fixed `overdue`/`due`/`within-sla` states reproduce. |
| `scripts/validate-mcp.sh` | Pass under Node `22.22.2`. The registry `sha512` matched `@adrkit/mcp@0.12.0`, package `src/` was byte-identical to the pinned commit, the 28 config and 28 stdio JSON-RPC assertions passed, and `docs/adr` ended byte-identical. |
| `scripts/validate-markers.sh --self-test` | Pass under Bun `1.4.0`: 26 assertions across 8 fixtures; 17 perturbations falsified 22 assertions, with the remaining 4 non-falsifiable by construction. |
| `scripts/validate-badge-reports.sh` | Pass under Bun `1.4.0`: 5 assertions. Both committed reports are byte-identical to regeneration at the new pin, so no `.adrkit/*.json` edit was needed. |
| `scripts/test-assert-managed-issue-body.sh` + `scripts/test-assert-no-issue-mutation.sh` | Pass. Every good fixture passed and every single-invariant mutation fixture was rejected. |
| `scripts/validate-spec-kit-extension.sh` | Pass, 49 assertions and zero failures at both Spec Kit `0.14.4` and `0.15.1`, using the sha256-pinned `spec-kit-v0.1.3` asset and `@adrkit/cli@0.12.0`; both mutation checks passed. |

The Spec Kit runs used a temporary committed snapshot of the exact repin diff,
so the `MUT-2` assertion checked a clean reference repository rather than being
weakened or skipped.

### What was *not* exercised at this pin (`2f19524f`)

- **Neither queue Action workflow was dispatched.** The executable queue bundle
  changed, so older live managed-issue and fail-closed results are historical
  evidence, not evidence for v0.12.0.
- **No pull-request or `main` CI run is claimed.** The results above are local
  observations, and the Bun-backed runs used `1.4.0` rather than CI's pinned
  `1.3.14`.
- **The hosted cloud-agent MCP setting was not read back.** Its update is
  user-reported; the checked-in canonical config and published package were
  validated here.
- **The new queue edge shape is absent from this corpus.** None of the three
  proposed records has a top-level `reviewBy` without a `review` block, so the
  new `item.tier-absent` branch was identified from the upstream diff, not
  observed in this repository's queue output.
- **Marker overflow and the new Action advisories were not exercised.** Existing
  fixtures stay below the 64-per-file and 10,000-per-batch declaration caps, and
  no live governing-decisions comment was rendered.
- **Dependency closures remain only behaviorally covered.** The top-level MCP
...
## Re-validation against `0bd7200b` (adrkit v0.10.0) (2026-08-25)

The repin from `e66b43dd` to `0bd7200b` spans six upstream commits. The release
focuses on CLI help, mistake recovery, shell completions, and TTY-aware
presentation. It does not change `packages/ci/**` or the queue-core source this
repository depends on, but it does change the CLI wrapper through which the
local queue validation runs.

**Scope caveat, stated up front.** Six validation surfaces were run locally. No
GitHub workflow was dispatched, no pull-request checks are claimed, and the
hosted cloud-agent MCP setting cannot be read back from this repository. The
task reported that hosted setting as already updated; this run updated and
validated the version-controlled canonical config but did not independently
observe the live UI state.

### What changed across `e66b43dd` → `0bd7200b`

The GitHub comparison API reports 45 changed files, 3,145 additions, and 341
deletions. The surfaces relevant here break down as:
...
| `packages/ci/dist/queue-action.js` | byte-identical (Git blob `04b63e4b…`) | Executable queue Action bundle unchanged |
| `packages/core/src/{queue/as-of.ts,check/index.ts}` | byte-identical (Git blobs `b88c6024…`, `ce64e5d5…`) | Fixed-date resolution and governed-path matching unchanged |
| `packages/cli/src/queue.ts` | +69 −28 | Adds shared help/error recovery and presentation handling around the same core report builder |
| `packages/cli/src/index.ts` plus new CLI modules | +383 −182 in `index.ts`; new registry, completion, error, presentation, and recovery modules | The release's main behavior change; most of it is outside this repository's queue JSON contract |
| `packages/mcp/src/server.ts` | +1 −1 | Server version moves `0.9.0` → `0.10.0` |
| `docs/adr/0028`, `docs/adr/0031` | status/evidence updates | Upstream decision-corpus changes do not alter this repository's corpus |

The unscoped `adrkit` package still returns `E404`. Registry metadata for the
published `@adrkit/cli@0.10.0` package exposes both `adr` and `adrkit` aliases;
the Spec Kit validation below installed the pinned published CLI and accepted
its reported version.

### What was run, and what it reported

| Validation | Observed result at `0bd7200b` |
|------------|--------------------------------|
| `scripts/validate-queue.sh` | Pass. `adr --version` reports `0.10.0`; 15 records lint with 0 errors and 0 warnings; all three tiers and the fixed `overdue`/`due`/`within-sla` states reproduce. |
| `scripts/validate-mcp.sh` | Pass. The registry `sha512` matched `@adrkit/mcp@0.10.0`, package `src/` was byte-identical to the pinned commit, the 28 config and 28 stdio JSON-RPC assertions passed, and `docs/adr` ended byte-identical. |
...
| `scripts/test-assert-managed-issue-body.sh` + `scripts/test-assert-no-issue-mutation.sh` | Pass. Every good fixture passed and every single-invariant mutation fixture was rejected. |
| `scripts/validate-spec-kit-extension.sh` | Pass, 49 assertions and zero failures at both Spec Kit `0.14.4` and `0.15.1`, using `@adrkit/cli@0.10.0`; both mutation checks passed. |

The first direct Spec Kit runs were made from the intentionally dirty repin
worktree. They reached all 49 assertions and failed only `MUT-2`, correctly
reporting the uncommitted pin edits. They are not counted as passing
validation. The passing runs used a temporary committed snapshot of the exact
diff and the tag, URL, digest, extension version, and CLI version from
`spec-kit-extension.yml`.

### What was *not* exercised at this pin (`0bd7200b`)

- **Neither queue Action workflow was dispatched.** The managed-issue and
  fail-closed live evidence remains at older pins. The upstream Action metadata,
  source entrypoint, and executable bundle are byte-identical to `e66b43dd`,
  but that is source identity, not a new live observation.
- **No pull-request or `main` CI run is claimed.** The results above are local
  observations only.
- **The hosted cloud-agent MCP setting was not observed.** Its UI state has no
  repository read API; only the checked-in canonical config and its package were
  validated here.
- **The new CLI presentation surface was not exhaustively dogfooded.** The run
  exercised `--version`, lint, deterministic queue JSON/Markdown, and the Spec
  Kit consumer paths. It did not separately test shell completion generation,
  typo suggestions, or interactive TTY color behavior.
- **Dependency closures remain only behaviorally covered.** The top-level MCP
  tarball is content-pinned; npm-resolved dependencies are not.

---

## Re-validation against `e66b43dd` (adrkit v0.9.0) (2026-08-24)

The repin from `d9ce9e18` to `e66b43dd` spans ten upstream commits. It changes
the CLI/core boundary around queue date resolution and the platform-specific
path normalization used by `adr check`, but it does not change the managed
...
The GitHub comparison API reports 171 changed files, 13,577 additions, and 664
deletions. The queue and governance surfaces relevant here break down as:

| File | Comparison | Relevance |
|------|------------|-----------|
| `packages/ci/queue/action.yml` | byte-identical (Git blob `57977f7f…`) | Queue Action metadata and inputs/outputs unchanged |
| `packages/ci/src/queue-action-entrypoint.ts` | byte-identical (Git blob `1f8ec39e…`) | Managed-issue orchestration entrypoint unchanged |
| `packages/ci/dist/queue-action.js` | byte-identical (Git blob `04b63e4b…`) | The executable dispatched below is unchanged |
| `packages/core/src/queue/as-of.ts` | new, +62 | Publishes the `resolveAsOf` contract previously private to the CLI |
| `packages/cli/src/queue.ts` | +2 −27 | Imports the core resolver instead of maintaining its own copy |
| `packages/core/src/check/index.ts` | +15 −2 | Normalizes separators only on platforms where backslash is a separator |
| `packages/ci/dist/index.js` | +3 −2 | Governing-decisions bundle changed; the queue bundle did not |
| `packages/{cli,mcp}/src/*` | +1 −1 each | Version strings move `0.8.0` → `0.9.0` |
| `packages/adrkit/**` | 6 files removed | Removes the unscoped forwarder npm declined to issue |

The unscoped `adrkit` package still returns `E404`, consistent with its upstream
removal. The published `@adrkit/cli@0.9.0` package was installed separately:
both `node_modules/.bin/adr --version` and
`node_modules/.bin/adrkit --version` printed `0.9.0`, and both aliases were
present.
...
| `scripts/validate-badge-reports.sh` | Pass, 5 assertions. Both committed reports are byte-identical to regeneration at the new pin, so no `.adrkit/*.json` edit was needed. |
| `scripts/test-assert-managed-issue-body.sh` + `scripts/test-assert-no-issue-mutation.sh` | Pass. Every good fixture passed and every single-invariant mutation fixture was rejected. |
| `scripts/validate-spec-kit-extension.sh` | Pass, 49 assertions and zero failures at both Spec Kit `0.14.4` and `0.15.1`, using `@adrkit/cli@0.9.0`; both mutation checks passed. |

Two initial direct Spec Kit invocations omitted the workflow's required
environment and exited during parameter validation before any assertion ran.
...
Run
[`32784931152`](https://github.com/mbeacom/adrkit-t018-dogfood/actions/runs/32784931152)
completed successfully against commit `d54c571` on the repin branch. The Action
step resolved `packages/ci/queue@e66b43dd87d49648a7b28630fa9ecfff65225f2e`;
the following verification step proved that issue
[#3](https://github.com/mbeacom/adrkit-t018-dogfood/issues/3) is the sole
managed issue, matches the Action's output, and satisfies every body assertion.
The refreshed body has SHA-256
`336128ee191b65787e92b2227b1840c9366e67fc8a570c0033d04374f589b214`
and `updatedAt: 2026-08-24T22:29:57Z`.

Issue #3 remains open by design: refreshing the operational queue is not an ARB
decision. It still reports the three proposed records and their real review
state; this repin does not fabricate approvals, resolve the objection on
`0014`, or change any governed record.

### What was *not* exercised at this pin (`e66b43dd`)

- **The fail-closed workflow was not re-dispatched.** Its queue Action
  entrypoint, metadata, and executable bundle are byte-identical to the previous
  pin, but the live invalid-input/no-write evidence remains from `c5dc677f`.
- **The governing-decisions Action is not claimed at this pin.** Its bundle
  changed, while `adr.yml` deliberately tracks mutable `packages/ci@main`;
  neither surface is evidence about the pinned queue Action.
- **The new core export was not consumed directly.** The built CLI exercised the
  resolver with the fixed `2026-07-21` date, but this repository did not add a
  separate application importing `resolveAsOf`.
- **The portable agent plugin was not exercised.** It is part of the upstream
  release, but no plugin install or host-compatibility claim is made here.
- **Dependency closures remain only behaviorally covered.** The top-level MCP
  tarball is content-pinned; npm-resolved dependencies are not.
...
At the current `2f19524f` pin the MCP position is different and better: the
